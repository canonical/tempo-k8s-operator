from scenario import Runtime, Scenario
...
from scenario.structs import CharmSpec, Context, Scene, State, event
...
def scenario():
    with mock.patch("charm.KubernetesServicePatch", lambda x, y: None):
        yield Scenario(CharmSpec.load(TempoCharm))


@pytest.fixture(params=(True, False))
def base_context(request):
    return Context(state=State(leader=request.param))


def test_start(scenario, base_context):
    out = scenario.run(Scene(event("start"), context=base_context))
