...
import ops
import pytest
from scenario import Container, State, PeerRelation, Relation


def test_monolithic_status_no_s3(context):
    state_out = context.run(
        "start", State(unit_status=ops.ActiveStatus(),
...
def test_scaled_status_with_s3_and_container_ready(context, leader):
    container = Container(name="tempo", can_connect=True, )
    state_out = context.run(
        container.pebble_ready_event, State(
            leader=leader,
            relations=[
                PeerRelation(
                    "tempo-peers",
