...
from opentelemetry.trace import get_current_span as otlp_get_current_span, get_tracer_provider
...
def _setup_root_span_initializer(charm: Type[CharmBase],
    ...
    def wrap_init(self: CharmBase, framework: Framework, *args, **kwargs):
        ...
        if not tempo:
            ...
            return original_init(self, framework, *args, **kwargs)
        ...
        def wrap_close():
            ...
            tp: TracerProvider = get_tracer_provider()
            tp.force_flush()
            tp.shutdown()
            original_close()
...
def trace_charm(tempo_endpoint: str, app_name: str = None) -> Callable[[_C], _C]:
    ...
    def trace_charm(charm: _C) -> _C:
        ...
        tempo = getattr(charm, tempo_endpoint, _notgiven)
        if tempo is _notgiven:
